      ...
      <div className='CardFull__info'>
        <div className='CardFull__block1'>
          <div className='CardFull__direction'>
            <h4 onClick={() => {navigate('../product'); dispatch(actve(propsNew))}}>{propsNew.info.name}</h4> 
            <BtnNewBest name='Bestseller' />
          </div>        
          <ProductPath />
        </div>        
        <div className='CardFull__block2'>
          <div className='CardFull__price-stars' style={propsNew.sum === 0 ? style1 : style2}>
            <ProductPrice price={propsNew.info.value_price} quantity={propsNew.info.quantity}/>
            <div className='CardFull__stars-reviews'>
              <Stars reward={+propsNew.info.grade_average}/>
              <span>{`/${propsNew.info.total_reviews} reviews`}</span></div>            
          </div>
          <div className='CardFull__orders'>{`${propsNew.info.total_orders} Orders`}</div>          
        </div>       
        <div className='CardFull__block3'>
          { propsNew.sum > 0 ? <ProductQuantityControl obj={propsNew} /> : <div className='CardFull__basket' onClick={() => handlerBasket()}><div /></div>}
