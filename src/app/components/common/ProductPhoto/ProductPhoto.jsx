  ...
  return (
    <div className='ProductPhoto'>
      <div className='Photo__slider'>
        <div
          className='Photo__slider_btn-up'
          onClick={moveUp} />
        <div className='Photo__slider_slides' style={{height}}>
          <div className='Photo__slider_list' style={{top, transition}} onTransitionEnd={change}>
            {slidesHalf1.map((p, index) => (           
            <img className='Photo__slider_slide' key={`${p}-${index}`} src={`${baseurl}/${p}`} alt="photo" />             
            ))}
        ...
        <div
          className='Photo__slider_btn-down'
          onClick={moveDown} />
      </div>
      <div className='Photo__image'>
        <img src={`${baseurl}/${slides[len + 1 - count]}`} alt="photo" />
      </div>
    </div>
  )
}
